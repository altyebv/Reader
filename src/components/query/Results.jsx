            ...
            <tr>
              {visibleColumns.map(col => (
                <th
                  ...
                  className={`px-3 py-2.5 text-xs font-bold text-gray-700 cursor-pointer hover:bg-gray-100 transition-colors whitespace-nowrap ${col.align === 'left' ? 'text-left' : 'text-right'}`}
                >
                  <div className={`flex items-center gap-1.5 ${col.align === 'left' ? 'flex-row' : 'flex-row-reverse'}`}>
          ...
          <tbody className="divide-y divide-gray-100">
            {transactions.length === 0 ? (
              ...
              transactions.map((transaction) => (
                <tr key={transaction.id} className="hover:bg-teal-50 transition-colors group">
                  {visibleColumns.map(col => {
                    ...
                    return (
                      <td
                        key={col.key}
                        className={`px-3 py-2.5 text-xs whitespace-nowrap ${col.align === 'left' ? 'text-left' : 'text-right'} ${col.key === 'amount' ? 'font-bold text-gray-900' : 'text-gray-700'} ${col.key === 'receiver_name' ? 'font-bold' : ''}`}
                        dir={col.dir}
                        style={{ fontFamily: col.dir === 'rtl' ? 'Cairo, sans-serif' : 'inherit' }}
                      >
                        {value}
                      </td>
                    );
